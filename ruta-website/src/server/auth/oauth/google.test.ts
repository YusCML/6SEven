...
describe('oauth state cookie', () => {
  it('round-trips state, verifier and returnTo', () => {
    ...
    const req = { cookies: { ruta_oauth: decodeURIComponent(value) } } as unknown as NextApiRequest;
  ...
  it('returns null for a missing or unreadable cookie', () => {
    expect(readOAuthState({ cookies: {} } as unknown as NextApiRequest)).toBeNull();
    expect(readOAuthState({ cookies: { ruta_oauth: 'garbage' } } as unknown as NextApiRequest)).toBeNull();
  });
